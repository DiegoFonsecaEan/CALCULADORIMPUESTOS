...
public class CalculadorImpuestos {
     ...
     */
        ...
        // Iteración de rangosImpuesto (teniendo en cuenta su longitud)
        for (int i=0; i<rangosImpuesto.length; i++) {
            // Se asigna cada rango de impuesto a la variable buscarRango.
            RangoImpuesto buscarRango = rangosImpuesto[i];
            // Se usa el método contieneA (Valor que se va a buscar en el rango),
            // sí devuelve true está en el rango.
            // Sino, vuelve a preguntar (iteración - Condición).
            if (buscarRango.contieneA(valor)){
                // Asigna el dato del rango al estar dentro del rango.
                rango = buscarRango;
                // Rompe la iteración al cumplir la condición del rango.
                break;
            }
        }
        return rango;
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        // Declarar variable porcentaje en coma flotante con el método darPorcentaje()
        // Ej: 1.0
        double porcentaje = rango.darPorcentaje();

        // Se calcula el valor del porcentaje por el precio del Vehículo actual
        // por el Porcentaje del rango, dividio entre 100.
    ...
    public Vehiculo buscarVehiculoMasCaro() {
        ...
        // Iteración de los vehículos por longitud.
        for (int i = 0; i < vehiculos.length; i++) {
            // Asignación de vehículoMasCaro por cada vahículo iterado.
            Vehiculo vehiculoMasCaro = vehiculos[i];
            // Sí el precio del vehículo es mayor al valor más alto.
            if (vehiculoMasCaro.darPrecio() > valorMasAlto){
                // Da el precio del vehículo con el precio más alto
                valorMasAlto = vehiculoMasCaro.darPrecio();
    ...
    public Vehiculo buscarVehiculoPorMarca(String marca) {
        ...
        // Iteración de vehículos por longitud
        for (int i = 0; i < vehiculos.length; i++) {
            // Asignación de Vehiculo por marca buscada
            Vehiculo VehiculoMarcaBuscada = vehiculos[i];
            // Comparar marca consultada con la de los datos.
            if (VehiculoMarcaBuscada.darMarca().equals(marca)){
                buscado = VehiculoMarcaBuscada;
                // Rompe la iteración sí iguala la marca.
                break;
            }
        }
        return buscado;
    ...
    public Vehiculo buscarVehiculoPorLinea(String linea) {
        ...
        // Iteración de vehículos por longitud
        for (int i = 0; i < vehiculos.length; i++) {
            // Asignación de Vehiculo por linea buscada
            Vehiculo VehiculoLineaBuscada = vehiculos[i];
            // Compara linea ingresada con la de los vehículos.
            if (VehiculoLineaBuscada.darLinea().equals(linea)){
                buscado = VehiculoLineaBuscada;
                // Rompe la iteración sí iguala la linea.
                break;
            }
